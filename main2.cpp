...
private:
    ...
    vector <QuestionAnswer> questionsanswers;
...
void Game::StartGame() {
    ...
    WINDOW* menu2win = newwin(20, maxX-12, maxY-30, 5 );
    WINDOW* question = newwin(5,maxX-12, maxY-40, 5);
    WINDOW* conclusion = newwin(60, maxX-100, maxY-60,50);
    box(question,0,0);
    box(menu1win, 0, 0);
    refresh();
    wrefresh(menu1win);
    ...
    if(highlight == 1) {
        ...
        str = "Do you want to play a PS game?";
        wclear(question);
        box(question, 0, 0);
        wattron(question, A_BOLD);
        mvwprintw(question, 2, maxX / 2 - floor((str.size()) / 2) - 5, str.c_str());
        ...
        if(highlight == 1){
            str = "Do you want to play a PC game?";
            wclear(question);
            box(question, 0, 0);
            wattron(question, A_BOLD);
            mvwprintw(question, 2, maxX / 2 - floor((str.size()) / 2) - 5, str.c_str());
            ...
            if(highlight == 1){
                ...
                while(1) {
                    ...
                    if(questionPoolPC.getLength()==questionsAsked.size()){
                        ...
                        box(conclusion,0,0);
                        str = "Your game is:";
                        mvwprintw(conclusion,4,maxX/2-(str.size()/2)-50,str.c_str());
                        str = conclusionPoolPC.getConclusion(maxindex);
                        wattron(conclusion,A_BOLD);
                        mvwprintw(conclusion,5,maxX/2-(str.size()/2)-50,str.c_str());
                        wattroff(conclusion,A_BOLD);
                        str ="Your Answers:";
                        mvwprintw(conclusion,7,maxX/2-(str.size()/2)-50,str.c_str());
                        for(int i = 0; i < questionsanswers.size();i++){
                            str = questionsanswers[i].question +"    " +questionsanswers[i].answer;
                            mvwprintw(conclusion,8+i,maxX/2-(str.size()/2)-50,str.c_str());
                        }
                        wrefresh(conclusion);
                        getch();
                        endwin();
                    ...
                    str = questionPoolPC.getQuestion(randNum);
                    wclear(question);
                    box(question,0,0);
                    wattron(question, A_BOLD);
                    mvwprintw(question, 2, maxX / 2 - floor((str.size()) / 2) - 5, str.c_str());
                    ...
                    string choices2[5] = {"YES","RATHER YES","I DON'T CARE","RATHER NOT","NO"};
                    box(menu2win,0,0);
                    wrefresh(menu2win);
                    while(1){
                        for (int i = 0 ; i < 5 ; i++){
                            if(i == highlight)
                                wattron(menu2win,A_REVERSE);
                            mvwprintw(menu2win,10, maxX/6+i*maxX/6-10,choices2[i].c_str());

                            wattroff(menu2win, A_REVERSE);

                        }

                        choice = wgetch(menu2win);
...
*/
                    ...
                    switch(highlight){
                        case 0:
                        {
                            questionsanswers.push_back(QuestionAnswer(str, "YES"));
                            valuesOld = compare(valuesOld,valuesNew);
                            break;
                        }
                        case 4:
                        {
                            questionsanswers.push_back(QuestionAnswer(str, "NO"));
                            for(int  i = 0; i < valuesNew.size();i++){
                        ...
                        case 1:
                        {
                            questionsanswers.push_back(QuestionAnswer(str, "RATHER YES"));
                            for(int  i = 0; i < valuesNew.size();i++){
                                valuesNew[i] *= 0.8;
                            }
                            valuesOld = compare(valuesOld,valuesNew);
                        ...
                        case 2:
                        {
                            questionsanswers.push_back(QuestionAnswer(str, "I DON'T CARE"));
                            break;
                        }
                        case 3:
                        {
                            questionsanswers.push_back(QuestionAnswer(str, "RATHER NOT"));
                            for(int i =0; i<valuesNew.size();i++){
                                valuesNew[i] = 1-valuesNew[i];
                                valuesNew[i]*=0.8;
                            }
...
*/
                    ...
                    if(differentToZero == 0){
                        box(conclusion,0,0);
                        str = "Sorry, it was impossible to determine a game for you. Try again.";
                        mvwprintw(conclusion,5,maxX/2-(str.size()/2)-50,str.c_str());
                        wrefresh(conclusion);
                        getch();
                        endwin();
            ...
            while(1) {
                ...
                if(questionPoolPS.getLength()==questionsAsked.size()){
                    ...
                    str = "Your game is:";
                    mvwprintw(conclusion,4,maxX/2-(str.size()/2)-50,str.c_str());
                    str = conclusionPoolPS.getConclusion(maxindex);
                    wattron(conclusion,A_BOLD);
                    mvwprintw(conclusion,5,maxX/2-(str.size()/2)-50,str.c_str());
                    wattroff(conclusion,A_BOLD);
                    str ="Your Answers:";
                ...
                str = questionPoolPS.getQuestion(randNum);
                wclear(question);
                box(question,0,0);
                wattron(question, A_BOLD);
                mvwprintw(question, 2, maxX / 2 - floor((str.size()) / 2) - 5, str.c_str());
...
int main(){
    ...
    values.push_back(Fuzzy("unlock",0.8));
    conclusionPoolPC.AddConclusion(Conclusion("Ori and the blind forest, Brothers: A Tale of Two Sons",values));
    values.clear();
    ...
    values.push_back(Fuzzy("chill",1));
    values.push_back(Fuzzy("unlock",0.8));
    conclusionPoolPS.AddConclusion(Conclusion("Ori and the blind forest, Brothers: A Tale of Two Sons",values));
    values.clear();
    ...
    values.push_back(Fuzzy("chill",0.5));
    values.push_back(Fuzzy("unlock",0.8));
    conclusionPoolPS.AddConclusion(Conclusion("Need for Speed Shift, Forza",values));
    values.clear();
    ...
    values.push_back(Fuzzy("chill",0.6));
    values.push_back(Fuzzy("unlock",0.6));
    conclusionPoolPC.AddConclusion(Conclusion("Portal 1 & 2, Portal Stories: Mel",values));
    values.clear();
    ...
    values.push_back(Fuzzy("chill",0.6));
    values.push_back(Fuzzy("unlock",0.6));
    conclusionPoolPS.AddConclusion(Conclusion("Portal 1 & 2, Portal Stories: Mel",values));
    values.clear();
